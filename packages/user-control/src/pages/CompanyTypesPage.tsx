...
interface CompanyType { id: number; code: string; description: string; isActive: boolean }
...
export default function CompanyTypesPage() {
  ...
  const [editing, setEditing] = useState<Partial<CompanyType>>(empty);
  const [saving, setSaving] = useState(false);
  const toast = useToast();

  const load = useCallback(() => {
    setLoading(true);
    userControlApi.getCompanyTypes({ page: 0, size: 100000, sort: 'id,asc' })
  ...
  const openEdit = (item: CompanyType) => { setEditing({ ...item }); setModalOpen(true); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true);
    try {
      if (editing.id) { await userControlApi.updateCompanyType(editing.id, editing); toast.success('Tipo de empresa atualizado com sucesso.'); }
      else { await userControlApi.createCompanyType(editing); toast.success('Tipo de empresa criado com sucesso.'); }
      setModalOpen(false); load();
    } catch { toast.error('Erro ao salvar tipo de empresa.'); } finally { setSaving(false); }
  };
  ...
  return (
    <>
      <DataTable title="Tipos de Empresas"
        ...
        rowActions={[{ icon: <Pencil size={14} />, label: 'Editar', variant: 'warning', onClick: (r) => openEdit(r as unknown as CompanyType) }]}
      />
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing.id ? 'Editar Tipo de Empresa' : 'Novo Tipo de Empresa'}>
