...
export default function CompanyTypesPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    userControlApi.getCompanyTypes({ page, size: pageSize, sort: 'id,asc' })
  ...
  return (
    <>
      <DataTable title="Company Types"
        ...
        rowActions={[{ icon: <Pencil size={16} />, label: 'Edit', variant: 'warning', onClick: (r) => openEdit(r as unknown as CompanyType) }]}
      />
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing.id ? 'Edit Company Type' : 'Add Company Type'}>
