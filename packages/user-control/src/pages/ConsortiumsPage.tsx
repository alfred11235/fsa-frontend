...
export default function ConsortiumsPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    userControlApi.getConsortiums({ page, size: pageSize, sort: 'id,asc' })
  ...
  return (
    <>
      <DataTable title="Consortiums"
        ...
        rowActions={[{ icon: <Pencil size={16} />, label: 'Edit', variant: 'warning', onClick: (r) => openEdit(r as unknown as Consortium) }]}
      />
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing.id ? 'Edit Consortium' : 'Add Consortium'}>
