...
interface Consortium { id: number; code: string; description: string; isActive: boolean }
const empty = { code: '', description: '', isActive: true };

export default function ConsortiumsPage() {
  const [data, setData] = useState<Consortium[]>([]);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(10);
  const [modalOpen, setModalOpen] = useState(false);
  const [editing, setEditing] = useState<Partial<Consortium>>(empty);
  const [saving, setSaving] = useState(false);
  const toast = useToast();

  const load = useCallback(() => {
    setLoading(true);
    userControlApi.getConsortiums({ page: 0, size: 100000, sort: 'id,asc' })
  ...
  const openEdit = (item: Consortium) => { setEditing({ ...item }); setModalOpen(true); };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true);
    try {
      if (editing.id) { await userControlApi.updateConsortium(editing.id, editing); toast.success('Consórcio atualizado com sucesso.'); }
      else { await userControlApi.createConsortium(editing); toast.success('Consórcio criado com sucesso.'); }
      setModalOpen(false); load();
    } catch { toast.error('Erro ao salvar consórcio.'); } finally { setSaving(false); }
  };
  ...
  return (
    <>
      <DataTable title="Consórcios"
        ...
        rowActions={[{ icon: <Pencil size={14} />, label: 'Editar', variant: 'warning', onClick: (r) => openEdit(r as unknown as Consortium) }]}
      />
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing.id ? 'Editar Consórcio' : 'Novo Consórcio'}>
